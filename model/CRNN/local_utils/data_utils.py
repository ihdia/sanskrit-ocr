...
class FeatureIO(object):
    ...
    """
    def __init__(self, char_dict_path=ops.join(os.getcwd(), 'model/CRNN/data/char_dict/char_dict.json'),
                 ord_map_dict_path=ops.join(os.getcwd(), 'model/CRNN/data/char_dict/ord_map.json')):
    ...
    def write_features(self, tfrecords_path, labels, images, imagenames):
        ...
        with tf.python_io.TFRecordWriter(tfrecords_path) as writer:
            for index, image in enumerate(images):
                ...
                #print("L"+" "+str(labels[index])+"L2"+" "+str(self.int64_feature(labels[index])))
                #print("IM"+" "+str(images.shape))
                #print("I"+" "+str(imagenames[index])+"I2"+" "+str(self.bytes_feature(imagenames[index])))
                print(type(image))
                print(type(self.bytes_feature(image)))
                #print(labels[index],self.int64_feature(labels[index]))
                #print(image,self.bytes_feature(image))
                #print(imagenames[index],self.bytes_feature(imagenames[index]))
...
class TextFeatureReader(FeatureIO):
    ...
    def read_features(tfrecords_path, num_epochs):
        ...
        images = tf.reshape(image, [32, 1000, 3])
