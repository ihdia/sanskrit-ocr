...
def sequence_loss_by_example(logits, targets, weights, labels, batch_size,
    ...
    with tf.name_scope(name, "sequence_loss_by_example",
        ...
        log_perp_list = []
        #logging.info("SeqLoss by Example Target: "+str(targets))
        #print("S:"+str(targets))
...
def sequence_loss(logits, targets, weights, labels, batch_size,
    ...
    #labels = tf.Print(labels,[labels],"labelss",summarize =  1000)
    #labels = tf.cast(labels,dtype=tf.float32)
    print("ZZZ"+str(labels))
    with tf.name_scope(name, "sequence_loss", logits + targets + weights):
        #labels = tf.Print(labels,[labels],"labelss",summarize =  1000)
        word_av_list = []
        w = []
        for i in range(batch_size):
            label = tf.gather_nd(labels, [i])
        ...
        cost = tf.reduce_sum(tf.multiply(tf.cast(norm_word_len, tf.float32),sequence_loss_by_example(
            logits, targets, weights,labels,batch_size,
...
def model_with_buckets(encoder_inputs_tensor,labels,  decoder_inputs, targets, batch_size, weights,
